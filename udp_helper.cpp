...
void UdpHelper::StartFunc(MessageHandler message_handler, char * buf, int len, int answer_mode, struct sockaddr_in remote_addr, socklen_t remote_len, int fd, std::mutex &mtx_for_fd, int delay_mode, double max_delay_secs, std::mutex &output_mutex, struct timeval oldtv, double send_time_out) {
    ResponseStruct response = message_handler(buf, len); 
    //ResponseStruct response;
    //response.message = (char *)"AK";
    //response.message_len = strlen("AK");

    //avoid using the same receiving fd concurrently
    //set time out
    Sleep(delay_mode, max_delay_secs, std::ref(output_mutex));
    if (answer_mode == ANSWER) {
        //char * response = (char *)"Ak";
        ...
        mtx_for_fd.lock();
        if (sendto(fd, response.message, response.message_len, 0, (struct sockaddr *)&remote_addr, sizeof(remote_addr)) < 0) {
            output_mutex.lock();
            perror("send Ak fails");
            output_mutex.unlock();
            //return -1;
        }
        //restore time
        if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &oldtv, sizeof(oldtv)) < 0) {
            perror("failed to restore time struct for receive.");
        }
        mtx_for_fd.unlock();
    }
}
...
int UdpHelper::ReceiveThread(int receive_window, const char *service_name, MessageHandler message_handler, int block_mode, int answer_mode, double max_delay_secs, int delay_mode, double send_time_out) {
    ...
    while (1) {
        ...
        if (recvlen > 0) {
            ...
            output_mutex.unlock();
            if (block_mode == BLOCK) {
                //StartFunc(message_handler, buf, recvlen, answer_mode, remaddr, remote_len, receive_window, delay_mode, max_delay_secs, oldtv, send_time_out, this);
                ...
                //std::thread t(StartFunc, message_handler, buf, recvlen, answer_mode, remaddr, remote_len, receive_window, delay_mode, max_delay_secs, oldtv, send_time_out);
                t.join();
            }
        }
    }
...
int UdpHelper::Sleep(int delay_mode, double max_delay_secs, std::mutex &output_mutex) {
    int random_delay = 0;
...
//return 1 for getting "AK" 
int UdpHelper::SendThread(MessageHandler message_handler, char *remote_ip, int remote_port, char *msg, int message_len, int block_mode, int answer_mode, double send_time_out, double max_delay_secs, int delay_mode, double recv_time_out) {
    ...
    if (answer_mode == ANSWER) {
        ...
        if (recvlen > 0) {
            ...
            output_mutex.unlock(); 
            ResponseStruct res = message_handler(buf, recvlen);
            if (strcmp(res.message, "AK") == 0) {
                ak = 1;
            }
        }
    }
...
//timeout_mode
int UdpHelper::Send(MessageHandler message_handler, char *remote_ip, int remote_port, char *message, int message_len, int block_mode, int answer_mode, double send_time_out, double max_delay_secs, int delay_mode, double recv_time_out) {
    //return SendThread(message_handler, remote_addr, remote_port, message, message_len, block_mode, answer_mode, send_time_out, max_delay_secs, delay_mode, recv_time_out);
    ...
    //send udp message 
    if (sendto(fd, message, message_len, 0, (struct sockaddr *)&remote_addr, sizeof(remote_addr)) < 0) {
        perror("send message fails");
        return -1;
    }
...
int UdpHelper::Receive(int receive_window, const char *service_name, MessageHandler message_handler, int block_mode, int answer_mode, double max_delay_secs, int delay_mode, double send_time_out) {
    ...
    return ReceiveThread(receive_window, service_name, message_handler, block_mode, answer_mode, max_delay_secs, delay_mode, send_time_out);
}
